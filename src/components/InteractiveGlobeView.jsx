...
import { getMagnitudeColor } from '../utils/utils.js'; // Directly import the utility
...
const InteractiveGlobeView = ({
    ...
    useEffect(() => {
        const internalHighlightedQuakeId = lastMajorQuake?.id; // Use lastMajorQuake from context
        let allPointsData = (globeEarthquakes || []).map(quake => { 
            const isHighlighted = quake.id === internalHighlightedQuakeId; // Use internal ID
            ...
            if (isHighlighted) {
                ...
                pointColor = getMagnitudeColor(magValue); // Use imported function
                pointAltitude = 0.03;
                ...
                pointColor = getMagnitudeColor(quake.properties.mag); // Use imported function
                pointAltitude = 0.01;
        ...
        if (previousMajorQuake && previousMajorQuake.id && previousMajorQuake.geometry?.coordinates && previousMajorQuake.properties) {
            ...
            allPointsData = allPointsData.map(p => {
                if (p.quakeData.id === previousMajorQuake.id) {
                    ...
                    if (p.quakeData.id === internalHighlightedQuakeId) { // Check against internal ID
                        return {
                            ...p, 
                    ...
                    return {
                        ...
                        color: getMagnitudeColor(prevMagValue), // Use imported function
                        altitude: 0.025, 
            ...
            if (!foundAndUpdated) {
                if (previousMajorQuake.id !== internalHighlightedQuakeId) { // Check against internal ID
                    allPointsData.push({
    ...
    }, [globeEarthquakes, lastMajorQuake, previousMajorQuake, activeClusters]); // Removed highlightedQuakeId and getMagnitudeColorFunc
    ...
    useEffect(() => {
        ...
        if (lastMajorQuake &&
            ...
            newRings.push({
                ...
                lng: coords[0],
                altitude: 0.02,
                color: () => getMagnitudeColor(mag), // Use imported function
        ...
        if (previousMajorQuake && 
            ...
            const baseColor = getMagnitudeColor(mag); // Use imported function
            newRings.push({
    ...
    }, [lastMajorQuake, previousMajorQuake, ringsData.length]); // Removed getMagnitudeColorFunc
