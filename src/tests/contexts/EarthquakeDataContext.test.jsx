...
describe('EarthquakeDataContext Reducer', () => {
  ...
  describe('WEEKLY_DATA_PROCESSED action', () => {
    ...
    // Define mock transformed_stats. These would be generated by the worker.
    // For test purposes, these are hardcoded but plausible for mockFeaturesWeekly.
    const mockDateToday = new Date(mockFetchTime);
    const dateStrings = Array.from({length: 7}).map((_, i) => {
        const d = new Date(mockFetchTime);
        // Ensure UTC dates are used for day calculation if getInitialDailyCounts in worker uses UTC
        d.setUTCDate(mockDateToday.getUTCDate() - i);
        // Using getUTCDate() for day to be consistent with setUTCDate()
        return `${d.toLocaleDateString('en-US', { month: 'short' })} ${d.getUTCDate()}`;
    }).reverse();

    const mockTransformedStats = {
      dailyCounts7Days: [
        { dateString: dateStrings[0], count: 3 }, { dateString: dateStrings[1], count: 2 },
        { dateString: dateStrings[2], count: 4 }, { dateString: dateStrings[3], count: 0 },
        { dateString: dateStrings[4], count: 0 }, { dateString: dateStrings[5], count: 0 },
        { dateString: dateStrings[6], count: 2 }, // Example counts
      ],
      magnitudeDistribution7Days: [ // Example distribution
        { name: '<1', count: 0, color: '#67E8F9' },
        { name: '1-1.9', count: 0, color: '#22D3EE' },
        { name: '2-2.9', count: 1, color: '#34D399' }, // e.g. w_quake1
        { name: '3-3.9', count: 5, color: '#FACC15' }, // e.g. w_quake2 and w_extra series
        { name: '4-4.9', count: 1, color: '#FB923C' }, // e.g. w_quake3
        { name: '5-5.9', count: 4, color: '#F97316' }, // e.g. w_quakeMajor, w_anotherMajor (pairs)
        { name: '6-6.9', count: 1, color: '#EF4444' }, // e.g. w_highMagDup (6.0 version)
        { name: '7+', count: 0, color: '#B91C1C' },
      ],
    };
    ...
    const action = {
      ...
      payload: {
        features: mockFeaturesWeekly, // Original features are still passed
        fetchTime: mockFetchTime,
        transformed_stats: mockTransformedStats // The new pre-calculated stats
      }
    };
    const updatedState = earthquakeReducer(initialState, action);
    ...
    it('should use dailyCounts7Days and magnitudeDistribution7Days from transformed_stats, and calculate sampledEarthquakesLast7Days', () => {
        // Assert that the state now uses the stats directly from the action payload
        expect(updatedState.dailyCounts7Days).toEqual(mockTransformedStats.dailyCounts7Days);
        expect(updatedState.magnitudeDistribution7Days).toEqual(mockTransformedStats.magnitudeDistribution7Days);

        // sampledEarthquakesLast7Days is still calculated client-side, so this part of the test remains similar
        expect(updatedState.sampledEarthquakesLast7Days).toBeInstanceOf(Array);
        // Optionally, verify its length based on SCATTER_SAMPLING_THRESHOLD_7_DAYS if mockFeaturesWeekly is large enough.
        // Need to define SCATTER_SAMPLING_THRESHOLD_7_DAYS in test scope if not already. Assuming it's 300 from previous context.
        const SCATTER_SAMPLING_THRESHOLD_7_DAYS = 300;
        const currentEarthquakesLast7Days = filterByTime(mockFeaturesWeekly, 7 * 24, 0, mockFetchTime);
        const expectedSampledLength = Math.min(currentEarthquakesLast7Days.length, SCATTER_SAMPLING_THRESHOLD_7_DAYS);
        expect(updatedState.sampledEarthquakesLast7Days.length).toBe(expectedSampledLength);
    });
  });
