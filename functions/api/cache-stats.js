...
async function collectCacheStatistics(db) {
  ...
  const queries = {
    ...
    activeEntries:
      "SELECT COUNT(*) as count FROM ClusterCache WHERE createdAt > datetime('now', '-1 hour')",
  ...
  const avgDataSizeKB = sizeResult?.avgDataSize
    ? sizeResult.avgDataSize / 1024
    : 0;
  const totalDataSizeMB = sizeResult?.totalDataSize
    ? sizeResult.totalDataSize / 1024 / 1024
    : 0;
  const cacheHealthScore =
    cacheEfficiency * 0.4 +
    (1 - expiredEntries / totalEntries) * 0.4 +
    (1 - Math.min(totalDataSizeMB / 1024, 1)) * 0.2;
  return {
    metadata: {
      timestamp: now,
    ...
    health: {
      ...
      status:
        cacheHealthScore > 0.8
          ? "Good"
          : cacheHealthScore > 0.6
            ? "Fair"
            : "Poor",
    ...
    storage: {
      ...
      maxEntrySizeKB: sizeResult?.maxDataSize
        ? Math.round((sizeResult.maxDataSize / 1024) * 100) / 100
        : 0,
      minEntrySizeKB: sizeResult?.minDataSize
        ? Math.round((sizeResult.minDataSize / 1024) * 100) / 100
        : 0,
    ...
    distribution: {
      ageGroups: (ageResult?.results || []).map((row) => ({
        ...
        percentage:
          totalEntries > 0
            ? Math.round((row.count / totalEntries) * 1e4) / 100
            : 0,
      })),
    },
...
function generateCacheRecommendations(metrics) {
  ...
  if (metrics.cacheEfficiency < 50) {
    recommendations.push({
      ...
      action:
        "Analyze cache access patterns and consider a longer TTL if appropriate.",
    });
    ...
    recommendations.push({
      ...
      action:
        "Analyze if a shorter TTL is feasible without impacting performance.",
    });
  }
  ...
  if (metrics.topParameters && metrics.topParameters.length > 0) {
    recommendations.push({
      ...
      priority: "low",
      message:
        "Consider implementing a cache warming strategy for frequently requested parameter sets.",
      action:
        "Create a scheduled task to pre-warm the cache with the top 10 most frequent parameter sets.",
    });
  }
...
async function onRequestDelete(context) {
  ...
  try {
    ...
    const deleteQuery =
      "DELETE FROM ClusterCache WHERE createdAt <= datetime('now', '-1 hour')";
    const deleteResult = await env.DB.prepare(deleteQuery).run();
    console.log(
      `Cache cleanup: deleted ${deleteResult.changes} expired entries`,
    );
    return new Response(
      JSON.stringify({
...
export { onRequestGet4 as onRequestGet, onRequestDelete };
