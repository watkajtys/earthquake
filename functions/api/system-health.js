async function onRequestGet5(context) {
  ...
  try {
    ...
    if (env.DB) {
      const dbTest = await env.DB.prepare(
        "SELECT COUNT(*) as count FROM EarthquakeEvents WHERE event_time > ?",
      )
  ...
  try {
    ...
    if (usgsResponse.ok) {
      ...
      healthReport.metrics.usgsDataFreshness =
        usgsData.metadata?.generated || null;
    } else {
      healthReport.components.usgsApi = {
        status: "degraded",
        responseTime: usgsDuration,
  ...
  try {
    if (env.DB) {
      ...
      `,
        ...
        .bind(new Date(Date.now() - 2 * 60 * 60 * 1e3).toISOString())
        .first();
      ...
      `,
        ...
        .bind(Date.now() - 2 * 60 * 60 * 1e3)
        .first();
      const recentDataCount = recentDataCheck?.count || 0;
      const lastDataUpdate = recentDataCheck?.lastEarthquake;
      const dataFreshness = lastDataUpdate
        ? (Date.now() - lastDataUpdate) / (1e3 * 60)
        : null;
      if (recentDataCount > 0 && dataFreshness < 10) {
        ...
        healthReport.components.scheduledTasks = {
          ...
          lastUpdate: lastDataUpdate
            ? new Date(lastDataUpdate).toISOString()
            : null,
        };
        healthReport.overall = "degraded";
      } else {
        healthReport.components.scheduledTasks = {
          status: "unhealthy",
          message:
            "No recent data updates detected - scheduled tasks may be failing",
          recentUpdates: recentDataCount,
          lastUpdate: lastDataUpdate
            ? new Date(lastDataUpdate).toISOString()
            : null,
  ...
  healthReport.metrics.healthScore = Math.round(
    (healthyCount / components.length) * 100,
  );
...
export { onRequestGet5 as onRequestGet };
