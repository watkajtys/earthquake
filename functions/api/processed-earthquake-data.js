...
const PROCESSED_DATA_KV_KEY = "latest_processed_data_v1";
...
export async function onRequestGet(context) {
  ...
  if (PROCESSED_DATA_KV) {
    try {
      const cachedData = await PROCESSED_DATA_KV.get(PROCESSED_DATA_KV_KEY);
      if (cachedData) {
        console.log(`[${sourceName}] Cache hit for processed data.`);
        return new Response(cachedData, {
          headers: { "Content-Type": "application/json", "X-Cache-Status": "hit" },
        });
      }
      console.log(`[${sourceName}] Cache miss for processed data.`);
    } catch (e) {
      console.error(`[${sourceName}] KV GET error: ${e.message}`, e);
  ...
  try {
    ...
    const dailyFeatures = rawDayData.features;
    ...
    allProcessedData.earthquakesLastHour = filterByTime(dailyFeatures, 1, 0, now);
    allProcessedData.earthquakesPriorHour = filterByTime(dailyFeatures, 2, 1, now);
    ...
    allProcessedData.hasRecentTsunamiWarning = earthquakesLast24Hours.some(q => q.properties.tsunami === 1);
    ...
    if(rawWeekData.features) majorQuakesFromAllFeeds.push(...rawWeekData.features.filter(q => q.properties.mag !== null && q.properties.mag >= MAJOR_QUAKE_THRESHOLD));
    if(rawMonthData.features) majorQuakesFromAllFeeds.push(...rawMonthData.features.filter(q => q.properties.mag !== null && q.properties.mag >= MAJOR_QUAKE_THRESHOLD));
    ...
    let earthquakesLast72Hours = filterByTime(weeklyFeatures, 72, 0, now);
    ...
    allProcessedData.earthquakesLast14Days = earthquakesLast14Days;
    ...
    allProcessedData.prev7DayData = filterMonthlyByTime(monthlyFeatures, 14, 7, now);
      ...
      try {
        ...
        if (env.PROCESSED_DATA_TTL_SECONDS) {
          ...
          if (!isNaN(parsedTtl) && parsedTtl > 0) {
            ...
            console.warn(`[${sourceName}] Invalid PROCESSED_DATA_TTL_SECONDS. Using default ${kvTtl}s.`);
          }
        }
        context.waitUntil(
            PROCESSED_DATA_KV.put(PROCESSED_DATA_KV_KEY, responseBody, { expirationTtl: kvTtl })
                .then(() => console.log(`[${sourceName}] Successfully cached processed data (TTL: ${kvTtl}s).`))
                .catch(e => console.error(`[${sourceName}] KV PUT error: ${e.message}`, e))
        );
      } catch (e) {
        console.error(`[${sourceName}] Error initiating KV PUT: ${e.message}`, e);
      }
    }
