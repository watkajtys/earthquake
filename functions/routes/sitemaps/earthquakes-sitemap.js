...
async function generatePaginatedEarthquakeSitemap(db, pageNumber) {
  ...
  try {
    // Fetch events that have enhanced data.
    const d1Results = await db.prepare(
      "SELECT id, magnitude, place, event_time, geojson_feature FROM EarthquakeEvents WHERE has_enhanced_data = TRUE ORDER BY event_time DESC LIMIT ? OFFSET ?"
