...
| `POST /api/calculate-clusters` | JSON ≤ **8 MiB**, at most **20,000** unique validated features, positive radius ≤ **500 km**, integer `minQuakes` from **1–20,000**. Spatial work is capped at **1,000,000** units. | Calculates clusters without D1 or queue writes. The original `clusters` and `cacheHit` response contract remains available to an authorized operator. |
...
The legacy calculation POST now passes through the same server credential gate as cluster registration, before reading its request body or running spatial work. The current frontend reads stored cluster summaries and does not call either POST. Without a configured `ADMIN_API_TOKEN`, both POST routes return **503** with `no-store` and do no calculation or database work. Operators who still need on-demand calculation must configure the existing server secret and send the bearer token from a trusted client. This contains expensive requests from cached older browsers; it does not by itself stop those browsers from sending repeated requests, and the 503s may continue appearing in error counts until those clients close or refresh.
